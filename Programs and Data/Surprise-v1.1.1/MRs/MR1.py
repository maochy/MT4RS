...
def exeMR(raw_trainset, raw_testset, mr_train_test_folder, assertInfo):
    ...
    while not flag3 or not flag4:
        ...
        while flag1 or flag2:
            flag1 = False
            flag2 = False
            chooseIndex1 = random.randint(0, len(raw_trainset) - 1)
            chooseIndex2 = random.randint(0, len(raw_trainset) - 1)
            ...
            chooseRow1 = raw_trainset[chooseIndex1][0]
            chooseCol1 = raw_trainset[chooseIndex1][1]
            chooseRow2 = raw_trainset[chooseIndex2][0]
            chooseCol2 = raw_trainset[chooseIndex2][1]

            for i in range(chooseIndex2):
                if i != chooseIndex1 and raw_trainset[i][0] == chooseRow1:
                    flag1 = True
                    break
                if raw_trainset[i][0] == chooseRow2:
                    flag2 = True
                    break

        for i in range(chooseIndex1):
            if raw_trainset[i][1] == chooseCol1:
                flag3 = True
            if raw_trainset[i][1] == chooseCol2:
                flag4 = True
            if flag3 and flag4:
                break

    chooseLine1 = raw_trainset[chooseIndex1]
    chooseLine2 = raw_trainset[chooseIndex2]
    raw_trainset[chooseIndex1] = chooseLine2
    raw_trainset[chooseIndex2] = chooseLine1

    with open(assertInfo, "w") as f:
        f.write("chooseLine1:" + str(chooseIndex1) + "\nchooseLineInfo1:" + str(raw_trainset[chooseIndex2]) + "\n")
        f.write("chooseLine2:" + str(chooseIndex2) + "\nchooseLineInfo2:" + str(raw_trainset[chooseIndex1]) + "\n")
...
def exeAssert(loop, folder):
    ...
    with open(folder + "assertInfo.txt", "a") as f:
        f.write("随机选取两行并交换位置，预测值应大致相同\n\n")
        f.write("userID\titemID\tresult_ori\t\t\tresult_mr\t\t\tresult_difference\n")
        for (u, i, r) in result_mr_dataList:
            if abs(float(r) - float(result_ori_dataDict[u][i])) > 0.1:
                count = count + 1
                f.write(u + "\t" +
                        i + "\t" +
                        result_ori_dataDict[u][i] + "\t\t" +
                        r + "\t\t" +
                        str(abs(float(r) - float(result_ori_dataDict[u][i]))) + "\n")
