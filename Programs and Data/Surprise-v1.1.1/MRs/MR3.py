...
def exeMR(raw_trainset, raw_testset, mr_train_test_folder, assertInfo):
    ...
    mr_ratings = []
    for i in range(len(raw_trainset)):
        mr_ratings.append((raw_trainset[i][0], raw_trainset[i][1], raw_trainset[i][2] + delta))

    mr_test = []
    for i in range(len(raw_testset)):
        mr_test.append((raw_testset[i][0], raw_testset[i][1], raw_testset[i][2] + delta))
...
def exeAssert(loop, folder):
    ...
    with open(folder + "assertInfo.txt", "a") as f:
        f.write("数加delta，预测值应大致为x + delta\n\n")
        f.write("userID\titemID\tresult_ori\t\t\tresult_mr\t\t\tresult_difference\n")
        for (u, i, r) in result_mr_dataList:
            if abs(float(r) - float(result_ori_dataDict[u][i]) - delta) > 0.1:
                count += 1
                f.write(u + "\t" +
                        i + "\t" +
                        result_ori_dataDict[u][i] + "\t\t" +
                        r + "\t\t" +
                        str(abs(float(r) - float(result_ori_dataDict[u][i]) - delta)) + "\n")
